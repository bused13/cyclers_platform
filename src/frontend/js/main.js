...
var start_location = [0,0], end_location = [0,0];
...
function initialize_maps() {
    ...
    var mapOptions = {
        center: new google.maps.LatLng(56.33837,-2.80055),
        zoom: 13,
...
function updateRoutes() {
    ...
    end_location = [routes[this.routeIndex].legs[0].end_location.lat(), routes[this.routeIndex].legs[0].end_location.lng()];
    start_location = [routes[this.routeIndex].legs[0].start_location.lat(), routes[this.routeIndex].legs[0].start_location.lng()];
...
//Save the information from the route planner and send it to the dashboard
var fromBoard = []
var toBoard = []
var distanceBoard = []
var dateTimeBoard = []
function save() {
    fromBoard.push(document.getElementById("from").value)
    toBoard.push(document.getElementById("to").value)
    distanceBoard.push(distance.text)

    var today = new Date();
    var date = today.getDate()+'-'+(today.getMonth()+1)+'-'+today.getFullYear();
    var time = today.getHours() + ":" + today.getMinutes();
    var dateTime = date+' '+time;
    dateTimeBoard.push(dateTime)

    localStorage.setItem("from", fromBoard);
    localStorage.setItem("to", toBoard);
    localStorage.setItem("distance", distanceBoard)
    localStorage.setItem("dateTime", dateTimeBoard)
